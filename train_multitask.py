...
def validation(dataloader, style_encoder, content_encoder, decoder, criterion, syn_or_real, aggregator):
    ...
    with torch.no_grad():
        ...
        for idx, x in enumerate(dataloader):
            ...
            # exit()
            encoded = aggregator([encoded_style, encoded_content, is_real])
...
def prediction(dataloader, content_encoder, style_encoder,
        decoder, target_alphabet , aggregator, syn_or_real, outfname,
...
if __name__ == '__main__':
    ...
    ## Bunch of datasets
    syn_train_dataset = Raw_Video_Dataset(args.synthetic_train_data_path,
        synthetic=True, training=True, video_length=300)
    syn_train_dataloader = data.DataLoader(syn_train_dataset,
    ...
    real_train_dataset = Raw_Video_Dataset(args.real_train_data_path,
        synthetic=False, training=True, video_length=300)
    real_train_dataloader = data.DataLoader(real_train_dataset,
        batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True,
        drop_last=True, worker_init_fn=_init_fn)
    real_train_cycle = cycle(real_train_dataloader)

    syn_test_dataset = Raw_Video_Dataset(args.synthetic_test_data_path,
        synthetic=True, training=False, video_length=300)
    syn_test_dataloader = data.DataLoader(syn_test_dataset,
        batch_size=128, num_workers=args.num_workers,
        shuffle=False, worker_init_fn=_init_fn)

    real_val_dataset = Raw_Video_Dataset(args.real_val_data_path,
        synthetic=False, training=False, video_length=300, random_seed=True)
    real_val_dataloader = data.DataLoader(real_val_dataset,
        batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False)
    bleu_dataloader_val = data.DataLoader(real_val_dataset,
        batch_size=1, num_workers=0, shuffle=False)

    real_test_dataset = Raw_Video_Dataset(args.real_test_data_path,
        synthetic=False, training=False, video_length=300, random_seed=True)
    real_test_dataloader = data.DataLoader(real_test_dataset,
        batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False)
    bleu_dataloader_test = data.DataLoader(real_test_dataset,
        batch_size=1, num_workers=0, shuffle=False)
    ...
    How was it typed?
    '''
    encoder = transformer.Encoder(128, args.hid_dim, args.n_layers,
        args.num_heads, args.emb_dim, args.dropout, device,max_length=300)
    ...
    content_discrim = transformer.Decoder(args.output_dim, args.hid_dim,
        args.n_layers, args.num_heads, args.emb_dim, args.dropout, device)
    #Style Classifier
    style_cls = transformer.Discrim()
    #Content Classifier
    content_cls = transformer.Decoder(args.output_dim, args.hid_dim,
        args.n_layers, args.num_heads, args.emb_dim, args.dropout, device)
    #Decoder
    decoder = transformer.Decoder(args.output_dim, args.hid_dim, args.n_layers,
        args.num_heads, args.emb_dim, args.dropout, device)

    #2N Discriminator
    joint_discrim = transformer.Discrim_Multi()

    #film to join style+ content
    aggregator = transformer.Aggregate()

    #Optimizers below
    optimizer_style_discrim = torch.optim.Adam(style_discrim.parameters(),
        lr=args.style_discrim_lr , weight_decay=1e-5)
    optimizer_content_discrim = torch.optim.Adam(content_discrim.parameters(),
        lr=args.content_discrim_lr, weight_decay=1e-5)
    optimizer_joint_discrim = torch.optim.Adam(joint_discrim.parameters(),
        lr=args.joint_discrim_lr, weight_decay=1e-5)
    ...
    optimizer_cls = torch.optim.Adam(
        [
            ...
            {"params": aggregator.parameters(), "lr": args.transformer_lr},
    ...
    style_encoder.load_state_dict(
        torch.load(args.pretrained_synthetic_transformer_encoder))
    ...
    content_discrim.load_state_dict(
        torch.load(args.pretrained_synthetic_transformer_decoder))
    ...
    for train_iter in range(args.max_iters):
        ...
        '''
        rs_rc_encoded = aggregator([real_style, real_content, True ])
        rs_sc_encoded = aggregator([real_style, syn_content,  True ])
        ss_rc_encoded = aggregator([syn_style, real_content, False])
        ss_sc_encoded = aggregator([syn_style, syn_content, False])
        ...
        if (train_iter % args.validation_iter) == 1:
            ...
            syn_val_loss = validation(syn_test_dataloader, style_encoder, content_encoder, decoder, seq_criterion, 'synthetic', aggregator)
            real_val_loss = validation(real_val_dataloader, style_encoder, content_encoder, decoder, seq_criterion, 'real', aggregator)
            ...
            bleu_val = prediction(bleu_dataloader_val, content_encoder, style_encoder, decoder, real_val_dataset.alphabet, aggregator, 'real', outfname = args.sentence_outputs_fname )

            test_bleu = prediction(bleu_dataloader_test, content_encoder, style_encoder, decoder, real_val_dataset.alphabet, aggregator, 'real', outfname = args.sentence_outputs_fname )
            ...
            aggregator_path = os.path.join(args.output_path, 'aggregator_{}.pth'.format(train_iter))
            ...
            if test_bleu > best_bleu:
                ...
                aggregator_path = os.path.join(args.output_path, 'aggregator_best.pth')
