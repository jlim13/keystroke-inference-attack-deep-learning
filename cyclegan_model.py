...
if __name__ == '__main__':
    ...
    parser.add_argument("--real_train_data_path", type = str, default = '../research/pytorch-CycleGAN-and-pix2pix/real2syn/keypress_cyclegan_multi_color/test_latest/images/train')
    parser.add_argument("--real_test_data_path", type = str, default = '../research/pytorch-CycleGAN-and-pix2pix/real2syn/keypress_cyclegan_multi_color/test_latest/images/test')
    parser.add_argument('--output_path', type = str, default = 'weights/cyclegan')
    ...
    parser.add_argument("--pretrained_cnn_extractor", type = str, default = 'best_source_cnn_extractor_.pth')
    ...
    out_fname = 'cyclegan_outputs.txt'
    ...
    for n,p in syn_encoder.named_parameters():
        if 'cnn' in n:
    ...
    # syn_encoder.load_state_dict(torch.load(os.path.join(args.output_path, 'encoder_best.pth' )))
    # decoder.load_state_dict(torch.load(os.path.join(args.output_path, 'decoder_best.pth' )))
    #
    ...
    for train_iter in range(args.max_iters):
        ...
        preds, _ = decoder(real_targ[:,:-1], real_encoded)
        preds = preds.contiguous().view(-1, args.output_dim)

        real_targ = real_targ[:,1:].contiguous().view(-1)

        iter_loss = seq_criterion(preds, real_targ)

        running_seq_loss += iter_loss.item()
        ...
        if (train_iter % args.validation_iter) == 1:
            ...
            print ("Real train loss {} | Real val loss {} | Bleu-1 Score {} | Best Bleu-1 Score {} |".format(this_seq_loss, real_val_loss, bleu, best_bleu))
